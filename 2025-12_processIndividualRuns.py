...
for initial_w, rho, beta, link_prob in param_combis:
    ...
    for event, pressure_strength in pressures.items():
        ...
        for seed in seeds:
            ...
            for eps, lam in fixadaptive:
                ...
                if run_idx == 0:
                    metrics["n_neighbours"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.uint16
                    )
                    metrics["personalBN_energy"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["personalBN_focal_energy"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["external_energy"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["social_energy"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["nr_balanced_triangles_tot"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["nr_balanced_triangles_focal"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["bn_avg_weighted_clustering"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["bn_betweenness_centrality"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["bn_expected_influence"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["bn_abs_meanedge_focal"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["bn_abs_meanedge_tot"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["abs_meanbelief_nonfocal"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                    metrics["focal_belief"] = np.zeros(
                        (Nruns, len(time_points), n_agents), dtype=np.float32
                    )
                ...
                for time_idx, (time, df_t) in enumerate(df.groupby("time", sort=True)):
                    ...
                    if store_beliefAndEdges:
                        for belief_idx, dim in enumerate(belief_dims):
                            ...
                            belief_results[dim][run_idx, time_idx, :] = dfi[dim].values
                        ...
                        for edge in edge_names:
                            ...
                            edge_results[edge][run_idx, time_idx, :] = dfi[edge].values
                    ...
                    # ------ BELIEFS --------------
                    metrics["abs_meanbelief_nonfocal"][run_idx, time_idx, :] = np.abs(
                        beliefs[
                            :, [i for i, b in enumerate(belief_dims) if b != focal_dim]
                        ]
                    ).mean(axis=1)

                    metrics["focal_belief"][run_idx, time_idx, :] = beliefs[
                    ...
                    # External Energy:
                    metrics["external_energy"][run_idx, time_idx, :] = (
                        -dfi["external_pressure_strength"].values
                        * beliefs[:, belief_dims.index(focal_dim)]
                    )
                    # Social Energy
                    ...
                    focal_edge_indices = [
                        idx
                        for idx, (a, b) in enumerate(params["edge_list"])
                        if a == focal_dim or b == focal_dim
                    ]
                    ...
                    for local_idx, (i, j) in enumerate(edge_idx_focal):
                        ...
                        focal_edge_energy += (
                            beliefs[:, i] * edges[:, global_edge_idx] * beliefs[:, j]
                        )
                    metrics["personalBN_focal_energy"][
                        run_idx, time_idx, :
                    ] = -focal_edge_energy
                    ...
                    metrics["nr_balanced_triangles_tot"][
                        run_idx, time_idx, :
                    ] = tri_balance
                    ...
                    metrics["nr_balanced_triangles_focal"][
                        run_idx, time_idx, :
                    ] = tri_focal_balance
                    ...
                    metrics["bn_abs_meanedge_tot"][run_idx, time_idx, :] = (
                        edge_abs.mean(axis=1)
                    )
                    metrics["bn_abs_meanedge_focal"][run_idx, time_idx, :] = np.abs(
                    ...
                    # metrics["bn_alpha"][run_idx, time_idx, :] = np.sign(prod).mean(
                    #     axis=1
                    # )
                    ...
                    )  # shape preserved for normalising later
                    ...
                    metrics["bn_avg_weighted_clustering"][run_idx, time_idx, :] = (
                        C.mean(axis=1)
                    )
    ...
    metrics["nr_balanced_triangles_tot"] = metrics["nr_balanced_triangles_tot"].astype(
        np.uint16
    )
    metrics["nr_balanced_triangles_focal"] = metrics[
        "nr_balanced_triangles_focal"
    ].astype(np.uint16)
    allData = {
        "n_neighbours": (("run", "time", "agent_id"), metrics["n_neighbours"]),
        "personalBN_energy": (
            ("run", "time", "agent_id"),
            metrics["personalBN_energy"],
        ),
        ...
        "bn_avg_weighted_clustering": (
            ("run", "time", "agent_id"),
            metrics["bn_avg_weighted_clustering"],
        ),
        "bn_betweenness_centrality": (
            ("run", "time", "agent_id"),
            metrics["bn_betweenness_centrality"],
        ),
        "bn_expected_influence": (
            ("run", "time", "agent_id"),
            metrics["bn_expected_influence"],
        ),
        "bn_abs_meanedge_focal": (
            ("run", "time", "agent_id"),
            metrics["bn_abs_meanedge_focal"],
        ),
        "bn_abs_meanedge_tot": (
            ("run", "time", "agent_id"),
            metrics["bn_abs_meanedge_tot"],
        ),
        "abs_meanbelief_nonfocal": (
            ("run", "time", "agent_id"),
            metrics["abs_meanbelief_nonfocal"],
        ),
    ...
    fname = f"processed_data/2025-12-30_modelAdaptiveBN_{condition_string}_results{'_detailed' if 'detailed' in filename else ''}_{'metricsOnly' if not store_beliefAndEdges else 'withBeliefsAndEdges'}.ncdf"
    ds.to_netcdf(fname)
