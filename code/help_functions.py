...
socialinfluenceMult = lambda wij, Wij, mu: (
    mu * (1 - abs(wij)) * Wij * wij if ~np.isnan(Wij) else 0
)
...
def initialise_socialnetwork(agent_list, identity, opinions, params):
    ...
    Args:
        ...
        identity (dict): Identity label of each agent indexed by agent ID.
    ...
    group_sizes = [
        (np.array(list(identity.values())) == party).sum() for party in parties
    ]
    ...
    agent_list = [
        ...
        if identity[agent] == "none"
        or identity[agent] == "other"
        or pd.isna(identity[agent])
    ]
...
def coherence(df, atts):
    coherence_edges = [
        df[i] * df[j] * df[f"({i},{j})"] for i, j in list(combinations(atts, 2))
    ]
...
def coherenceOther(bnEdges, opinion):
    ...
    atts = (
        ...
        else (opinion.index if type(opinion) == pd.Series else None)
    )
    coherence_edges = [
        opinion[i] * opinion[j] * bnEdges[f"({i},{j})"]
        for i, j in list(combinations(atts, 2))
    ]
...
def glauber_probabilities_withSocial(
    ...
    - beliefs: Current belief states for all attributes.
    - BN_ag: A dictionary with interaction weights between attribute pairs.
    # SET TO 1- beta_pers: attention parameter controlling randomness; 1/T to personal dissonance
    # SET TO 1- beta_soc: attention parameter controlling randomness; 1/attention to social dissonance
    - atts: List of all attributes.
