...
for T in [0.1]:
    eps, mu, lam, Temp, inf, net = (0.05,0.4,0.0, T, "copy", "observe-neighbours")
    ...
    fig, axs = plt.subplot_mosaic([["a", "a2", "x","b", "b2"], ["c","c","c", "c", "c2"]], sharey=True, gridspec_kw={"width_ratios":[1,0.2,0.05,1,0.2]})
    axs["x"].axis("off")
    i, j = ("a", "b") 
    e = f"({i},{j})" if f"({i},{j})" in edgeNamesTuple else f"({j},{i})"
    for ax, feature in zip(["a", "b", "c"], [i, j, e]) :
...
# muarr = [0.05,0.2, 0.4]
# T = 1
# compare(0.4, 0.2,0.05,T,"copy", "observe-neighbours", epsarr, muarr, seed, M, n_ag, ngroups, indegree, outdegree, belief_jump, resultsfolder,  edgeNamesTuple, atts)