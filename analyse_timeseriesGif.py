...
agss = [[67], [65], [61]]
cols = [cmapResponse[r] for r in ["resistant", "compliant", "resilient"]]
# 61 resilient
# 65 compliant
# 67 resistant
for ags, col in zip(agss, cols):
    Tfinal = 200
    for T in np.arange(0, 201, 1)[1:]:
        window = 0
        for adaptive, w0 in zip([True], [0.2]):
            ...
            for n, ax in enumerate([axs["t"], axs[f"a{T0}"]]):
                ax.text(
                    -0.12 if n == 0 else 0,
                    0.975,
    ...
    for t in np.arange(1, 201):
        im = Image.open(resultsfolder + f"_t{t}.png")
        images.append(im)
    last_frame = len(images)
    for x in range(0, 9):
        im = images[last_frame - 1]
        images.append(im)
    gifname = resultsfolder + ".gif"
    images[0].save(
        gifname,
        save_all=True,
        append_images=images[1:],
        optimize=True,
        duration=40 if ag_i == "all" else 50,
        loop=0,
    )
    print(gifname)
# %%
