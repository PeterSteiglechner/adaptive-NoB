...
for init_w, eps, mu, fixedBNat100 in [
    (init_w, 0.0, 0.0, False) for init_w in [0.2, 0.8]
] + [
    (init_w, eps, mu, fixedBNat100)
    for eps in [1.0]
    for mu in [0.0]
    for init_w in [0.2]
    for fixedBNat100 in [False, True]
]:
    for s in s_exts:
...
if show:
    ...
    examplesimadaptive = pd.read_csv(
        "simOut/detailed/sim_link_prob0.10_init_w0.20_beta3.00_rho0.33_eps1.00_mu0.00_ext_strength4_seed1_detailed.csv"
    )
