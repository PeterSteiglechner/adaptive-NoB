...
for ax, adaptive in zip(axs[2:], [False, True]):
    ...
    # sns.stripplot(subset, ax=ax, x="s_ext_log2", hue="response", y="normalized_count", palette=cmap, hue_order=responses, legend=False, size=1, )

    # sns.boxplot(subset, ax=ax, x="s_ext_log2", hue="response", y="normalized_count",palette=cmap, hue_order=responses, legend=False, whis=0, fliersize=0, linewidth=0)
...
axs[0].text(0.02,1.02,"example simulation (adaptive, $s=$"+f"{ext_pressure_strength}{', smoothed' if window>0 else ''})", transform=ax_main.transAxes, ha="left", va="bottom", fontsize=smallfs)
...
plt.savefig(
     f"figs/results_{condition_string}_proportions_evaluatedAtT={eval_time}_bars.png", dpi=300
)
...
axs[-1,0].set_xlabel("")
axs[-1,-1].set_xlabel("")
axs[0,0].text(0.02,1.02,f"example simulation (adaptive{f', smoothed ({window} steps)' if window>0 else ''})", transform=axs[0,0].transAxes, ha="left", va="bottom", fontsize=smallfs)
import string
for n, ax in enumerate(axs.flatten()): 
    ax.text(0.025, 0.975, string.ascii_uppercase[n], fontsize=12, fontdict={"weight":"bold"},va="top", ha="left", transform=ax.transAxes)
fig.subplots_adjust(left=0.06, right=0.98, top=0.94, bottom=0.12, wspace=0.1)
plt.savefig("figs/2026-01-26_responses_over_sext.png", dpi=600)
